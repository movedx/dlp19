...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = np.empty((N, F, H_out, W_out), dtype=float)
    ...
    for n in range(N):
        for i in range(0,H,stride):
            for j in range(0,W,stride):
                ...
                for f in range(F):
                    out[n,f,int(i/stride),int(j/stride)] = np.sum(window * w[f]) + b[f] 
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(0,H,stride):
        for j in range(0,W,stride):
            for n in range(N):
                x_rf = x_pad[n, :, i:i+HH, j:j+HH]
                for f in range(F):
                    dy = dout[n, f, int(i/stride), int(j/stride)]
                    dw[f] += dy * x_rf
                    db[f] += dy
                    dx[n, :, i:i+HH, j:j+HH] += dy * w[f]
...
def max_pool_forward_naive(x, pool_param):
    ...
    out = np.empty((N, C, H_out, W_out), dtype=float)
    
    for i in range(0,H,stride):
        for j in range(0,W,stride):
            window = x[:, :, i:i+pool_height, j:j+pool_width] 
            out[:, :, int(i/stride), int(j/stride)] = np.max(window, axis=(2, 3)) 
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(0,H,stride):
                for j in range(0,W,stride):
                    pool_window = x[n,c,i:i+pool_height, j:j+pool_width]
                    ...
                    dx[n, c, i:i+pool_height, j:j+pool_width] = dpool_window * dout[n, c, int(i/stride), int(j/stride)]
